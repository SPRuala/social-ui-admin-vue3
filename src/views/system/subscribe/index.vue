<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="医疗类型" prop="treatmenttype">
        <el-select
          v-model="queryParams.treatmenttype"
          placeholder="请选择医疗类型"
          clearable
          class="!w-240px"
        >
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['system:subscribe:create']"
        >
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="医疗类型" align="center" prop="treatmenttype" />
      <el-table-column
        label="预约时间"
        align="center"
      ...
      <el-table-column label="预约者名称" align="center" prop="name" />
      <el-table-column
        label="创建时间"
        align="center"
        prop="createTime"
      ...
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['system:subscribe:update']"
          >
            编辑
          </el-button>
          <el-button
            ...
            v-hasPermi="['system:subscribe:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <SubscribeForm ref="formRef" @success="getList" />
</template>
...
/** 医疗预约 列表 */
defineOptions({ name: 'Subscribe' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<SubscribeVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await SubscribeApi.getSubscribePage(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await SubscribeApi.exportSubscribe(queryParams)
    download.excel(data, '医疗预约.xls')
  } catch {
  } finally {
